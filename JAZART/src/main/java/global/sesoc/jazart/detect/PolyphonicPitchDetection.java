...
public class PolyphonicPitchDetection {
	...
	public ArrayList<Integer>[] hbIndices; /*
	...
	public ArrayList<Integer>[] f0indHarm; /*
	...
	public PolyphonicPitchDetection(String filepath) { /* Constructor */
		 ...
		 */
		for (int b = 0; b < 32; ++b) {
			cb[b] = 229.0 * (Math.pow(10.0, (((double) (b + 1.0)) / 21.4)) - 1.0); // frequency
																					// division
		}
		/*
		...
		for (int k = 0; k < f0index.length; ++k) {
			...
			for (int h = 0; h < harmonics; ++h) {
				ArrayList<Integer> tempInd = find(freq, f0cands[k] * ((double) h + 1.0) - halfBinWidth,
		...
		/*
		 * Capture capture = new Capture(16, this); Thread captureThread = new
		 * Thread(capture, "captureThread"); captureThread.start();
		 */

		WavFileHelper wfh = new WavFileHelper(16, this);
		Thread thread = new Thread(wfh, "wavThread");
		thread.start();
	}
