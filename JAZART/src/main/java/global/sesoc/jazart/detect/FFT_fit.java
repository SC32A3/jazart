...
public class FFT_fit {
	...
	public static double[] reconstruct(Complex[] y, int N) {
		...
		for (int i = 0; i < length; ++i) { // reconstruct the signal
			...
			t = ((double) i) / (length);
			for (int j = 1; j < N; ++j) {
				reconstructed[i] += 2.0 * y[j].re() / ((double) length) * Math.cos(((double) j) * 2 * Math.PI * t)
						- 2.0 * y[j].im() / ((double) length) * Math.sin(((double) j) * 2 * Math.PI * t);
			}
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < N; i++) {
			// x[i] = new
			// Complex(0.5*Math.sin(((double)i)*5*2*Math.PI/((double)N))+2, 0);
