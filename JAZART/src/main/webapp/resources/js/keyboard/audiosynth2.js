...
!function(){
	...
	setPub('generate', function(sound, note, octave, duration) {
		...
		if(typeof(this._fileCache[sound][octave-1][note][time])!='undefined') {
			...
			console.log('1: '+this._notes[note]); //
			console.log('2: '+this._sampleRate); // 44100
			console.log('3: '+this._volume); // 16384
			console.log('4: '+this._channels); // 1
			console.log('5: '+this._bitsPerSample); // 16
...
function AudioSynthView() {
	...
	var fnCreateKeyboard = function(keyboardElement) {
		// Generate keyboard
		// This is our main keyboard element! It's populated dynamically based
		// on what you've set above.
	...
	// 현우테스트
	var fnPlayNote2 = function(note, octave) {
		alert('note: '+note);
		alert('octave: '+note);
...
$(function() {
	...
	var toPiano = document.querySelector('.toPiano'); // 스톱버튼
	...
	if (navigator.getUserMedia) {
		...
		var onSuccess = function(stream) { // 성공시 스트림을 받아서
			...
			// 데이터가 사용가능하면 빈배열에 데이터를 넣어라..?! 레코더에 값이 닮기면 청크에 값넣기
	...
	function visualize(stream) {
		...
		// 미디어스트림소스에 연결 (스트림<->애널라이저(데이터가담기는))
		// 오디오 세팅인듯..
		...
		function draw2() {
			// 애니메이션 프레임
			// 애널라이저재료dataArray를 애널라이저의 시간 바이트로 쪼갬(0-255인듯)
			// x좌표에 버퍼로 나눈 slicewidth만큼 그려줌
			...
			canvasCtx.lineWidth = 2; // 선두깨인거같은데
