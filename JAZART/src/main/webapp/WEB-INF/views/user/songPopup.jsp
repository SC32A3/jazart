...
<script type="text/javascript">
	function input(title, nickname, songfile, songnum) {
		var myTitle = $('#myTitle');
		var myNickname = $('#myNickname');
		var myLink = $('#playerlink').attr("href"); //파일명 송파일
		var myImage = $('#playerimage').attr("data-bgimage"); //이미지명 송닉네임

		myTitle.html(title);
		myNickname.html(nickname);
		/* myLink = 'download?type=music&data='+songfile;
		myImage = 'download?type=song&data='+songnum; */
	}
</script>
</head>
<body>
			...
			<ul
				...
				<c:forEach var="item" varStatus="status" items="${playlist}">
					<li class="qt-channel"><a href="#!" class="qt-ellipsis"
						...
						onclick="javascript:input('${item.song_title}','${item.song_nickname}','${item.song_file}', '${item.songnum}')">
